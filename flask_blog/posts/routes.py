...
def allpost():
    ...
    posts = Post.query.order_by(Post.date_posted.desc()).paginate(page=page,
                                                                  per_page=5)
    return render_template('allpost.html', posts=posts, title='Все записи')
...
def new_post():
    ...
    if form.validate_on_submit():
        post = Post(title=form.title.data, content=form.content.data,
                    author=current_user)
        print(form.title.data, form.content.data, current_user)
        db.session.add(post)
        db.session.commit()
...
def post_edit(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Ваш пост обнавлён!', 'success')
